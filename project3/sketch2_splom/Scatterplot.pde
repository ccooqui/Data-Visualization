...
class Scatterplot extends Frame {
  ...
  Scatterplot( Table _data, String _dataVal1, String _dataVal2, Float _w1, Float  _h1) { //<>//
    data = _data;
    dataVal1 = _dataVal1;
    dataVal2 = _dataVal2; //<>//
    w1 = _w1;
  ...
  void draw() {
    ...
    float beginWidth = w1;
    float endWidth = w1+120;

    
    //heights for graph frame
    float beginHeight = h1;
    float endHeight = h1+120;
    println(endWidth);
    println(endHeight);
    ...
    for (TableRow row : data.rows()) {
       ...
       ellipse(map(row.getFloat(dataVal2), colStartRange, colEndRange, beginWidth, endWidth), map(row.getFloat(dataVal1), rowStartRange, rowEndRange, endHeight, beginHeight), 1.5, 1.5);
       //println(scaleH);
